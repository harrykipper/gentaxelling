...
globals [immigration districts file-name-pop file-name-entropy file-name-prices file-name-income file-name-world file-name-allured version save-directory
  ...
  take-measurements ; when to save data
  ...
  medianprices median-income mostrefugee have-refugees? howmanySlums howmanyWellMaintained howmanyOvercrowded ; new reporters for analysis
  housing-waiting-list file-name-donut totalcondition file-name-repair file-name-results
]
...
to setup
  ...
  set disp? false
  set firstallure 0
  set take-measurements [10 200 400 600 800 1000]
...
to set-last-renovated
  if condition <= slum-threshold [set last-renovated 120 + random 120]
  if condition > slum-threshold and condition <= 0.25 [set last-renovated 60 + random 120]
...
to go
  ...
  tick
  if paperstuff? and member? ticks take-measurements [
  update-reports
  ;save-screenshots
  ]
  if paperstuff? and (ticks = end-tick - 1) [update-reports]  ; don't save screenshots at end-tick - 1

  if ticks = end-tick [
    ;if paperstuff? [save-screenshots]
    if write-csv? or paperstuff? [save-data]
...
to update-dissonance
  ask citizens [
    ifelse [condition] of patch-here < slum-threshold or mean [condition] of other patches in-radius 2 < slum-threshold
...
to update-propensity
  ask citizens [
    ...
    if (enable-culture and dissonance > tolerable-dissonance) [
      set mobility-propensity mobility-propensity * 1.50
...
to weak-relocate-to [place]
    let ideal patches with [(price <= [income] of myself) and (count citizens-here = 0) and (neighbourhood = place) and (condition >= (mean [condition] of patches - (mean [condition] of patches * slum-threshold )))]
    ifelse any? ideal
    [move-to min-one-of ideal [dist]]
    [ let testbed n-of 5 patches
      let secondbest patches with [(price <= [income] of myself) and (count citizens-here = 0) and (condition >= (mean [condition] of testbed - (mean [condition] of testbed * slum-threshold )))]  ;; if we can't find a place we like then we move to one we can afford
      ifelse any? secondbest
      [move-to min-one-of secondbest [dist]]
...
to-report is-slum?
  if condition <= slum-threshold
  [report true]
  report false
end

to update-reports
  set medianprices lput median [price] of patches with [condition >= slum-threshold] medianprices
  set median-income lput median [income] of citizens median-income
  ;set howmanySlums lput (count patches with [condition <= slum-threshold] / count patches) howmanySlums
  set howmanySlums lput (count patches with [is-slum?] / count patches) howmanySlums  ; since is-slum? combines patch-condition and patch-population criteria, there's a case to use instead just condition <= slum-threshold here
  set howmanyWellMaintained lput (count patches with [condition > 0.5] / count patches) howmanyWellMaintained
end

; =============================== PRICE GAPS ====================================

; We test 4 mechanisms to establish the "highest and best use" of a residential unit defined as follows:

; 1. Lnd: ==
;mean of non empty patches in radius 2 OR
;mean of patches in radius 2 * 0.65
;+ premium

; 2.New: ==
;max of neighbours8 OR
;(mean of neighbourhood + premium)

; 3. Mean: ==
;mean of neighbours8 OR mean of neighbourhood
;+ premium

; 4. Max: ==
;max of neighbourhood OR max of neighbours8
;+ premium
...
to set-gaps-lnd ; mean of inhabited neighbourhood in radius 2. Reduced if not inhabited
  let whichprice 0
  let neigh-price 0
  let areaprice 0
...
to save-data
  ...
  set file-name-results (word save-directory "gentax-" version ".csv")  ; then save one big file with all the measures we want
  let run-number 0
  if behaviorspace-run-number != 0 [set run-number behaviorspace-run-number]
...
;  file-print (word run-number ";" Kapital ";" gaps ";" profit-threshold ";" immigration-rate ";" totalcondition ";" count patches with [condition >= 0.49] ";" medianprices ";" median-income ";" median [income] of citizens-on patches with [distancexy 0 0 < 4] ";" median [income] of citizens-on patches with [distancexy 0 0 >= 4 and distancexy 0 0 < 8] ";" median [income] of citizens-on patches with [distancexy 0 0 >= 8])
...
## Version 0.2.18
Added culture checks for better comparison
Added LND gap mechanism
...
<experiments>
  ...
  <experiment name="pnas" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
